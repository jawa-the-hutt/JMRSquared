...
router.post("/update/profile/picture/for/:adminID", async (req, res) => {
  var pic = req.body.profilePic;
});

router.post("/update/profile/for/:adminID", async (req, res) => {
        ...
        .then(business => {
          ...
          business.save(async err => {
            ...
            var payload = helper.makePayload(
              `${business.name} got a new ${admin.role}`,
              `${admin.userName} is added as a ${admin.role} for ${
                business.name
              }`, {
                link: `/profile/view`,
                props: admin._id,
            ...
            if (!req.body.partnerID) {
              try {
                var message = `Hi ${admin.fullName} ${
                  admin.userName
                } and welcome to JMRSquared. \n\nYou have been added as a ${
                  admin.role
                ...
                if (smsResponse) {
                  var payload2 = helper.makePayload(
                    `Your new ${admin.role} was notified successfully`,
                    `We sent an sms to ${
                      admin.fullName ? admin.fullName : admin.userName
                    } with their login details to the system.`, {
                      link: "",
                      props: "",
                      deactive: "true"
                    }
                ...
                var payload2 = helper.makePayload(
                  ...
                  `We were unable to sent an sms to ${
                    admin.fullName ? admin.fullName : admin.userName
                  } that contains their login details to the system.`, {
                    link: "",
                    props: "",
                    deactive: "true"
                  }
...
router.get(
  "/get/previous/devices/for/:adminID",
  auth.required,
  (req, res, next) => {
    var adminID = req.params.adminID;
    Admin.findById(adminID)
      .then(admin => {
        ...
        var devices = admin.deviceTokens
          .filter(d => d && d.deviceInfo)
          .map(device => {
            return {
              icon: "mdi-cellphone",
...
router.post("/device/token/add", function (req, res) {
  ...
  Admin.findById(adminID)
    .then(admin => {
      ...
      var exist = admin.deviceTokens.some(
